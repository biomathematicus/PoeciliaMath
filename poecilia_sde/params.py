...
class BaseParams:
    ...
    a: float = 0.8             # proportion host females in host progeny
    gamma_o: float = 1.0       # initial per-encounter acceptance probability (no preference)
    gamma_inf: float = 0.2     # asymptotic per-encounter acceptance probability (strong discrimination)
    r: float = 5.0             # sigmoid shift parameter
    v: float = 60.0            # discrimination speed (nondim); v=60 fast (coexistence), v=6 slow (extinction)
    # Initial conditions (non-dimensional: u = u_tilde / K)
    f0: float = 100/300        # initial host females (nondim)
    m0: float = 100/300        # initial host males (nondim)
    p0: float = 10/300         # initial parasite females (nondim)
...
class DimensionalParams:
    ...
    gamma_inf: float = 0.2
    r: float = 5.0
    v_slow: float = 0.02       # slow discrimination speed: leads to extinction
    v_fast: float = 0.20       # fast discrimination speed: leads to coexistence
    f0: float = 100.0
    m0: float = 100.0
    p0: float = 10.0
    t_end: float = 200.0
