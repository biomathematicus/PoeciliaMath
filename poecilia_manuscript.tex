...
contracts on this face when $\nabla \cdot \mathbf{F} < 0$ there.  Here $\mathbf{F}$ denotes the right-hand side of the non-dimensional RODE system~\eqref{eq:PoeciliaF}--\eqref{eq:PoeciliaP}.
...
\end{equation}
where $Q$ is a polynomial in $f$, $m$, $p$ that collects all state-dependent terms (verified symbolically; Appendix~A).  A key algebraic fact is that on the constraint surface $f + m + p = 1$
...
\end{equation}
Since this is the maximum value of $\nabla \cdot \mathbf{F}$ on the face $f+m+p=1$, the divergence is negative on that face if and only if
\begin{equation}
