...
\title{The Sperm Parasite Paradox: \\Stochastic Stabilization in Gynogenetic Complexes}
...
%% =====================================================================
\begin{abstract}
The Amazon molly (\textit{Poecilia formosa}) is a gynogenetic species that reproduces by sperm parasitism: females require sperm from males of a host species to initiate embryo development, but the paternal genome is not incorporated into offspring.  This creates a gynogenetic system in which parasite females drive the host population toward feminization and potential extinction.  The puzzle of continued coexistence between \textit{P.~formosa} and its hosts has motivated mathematical models incorporating population-dependent male mate discrimination and environmental stochasticity.

We present a three-population model (host females~$f$, males~$m$, parasite females~$p$) with logistic growth and a sigmoid discrimination function, analyzed under three stochastic frameworks: (i)~a Random ODE (RODE) with piecewise-constant parametric noise, (ii)~an It\^{o} stochastic differential equation (SDE) with multiplicative geometric noise, and (iii)~a Stratonovich SDE with the same noise structure but different stochastic calculus convention.  Each SDE formulation is studied under two noise structures---common environmental noise (single Wiener process) and independent multiplicative noise (per-population Wiener processes)---yielding five formulations in total.
...
$\tilde{p}$ is small, males encounter few parasite females and mate
indiscriminately ($\gamma \approx \gamma_o$); when $\tilde{p}$ is large,
males strongly avoid parasite females ($\gamma \approx \gamma_\infty$)
...
\text{constant}$.  The equilibrium structure and extinction conditions
are developed in Section~\ref{sec:extinction}.

When $\gamma p > f$, the term $f - \gamma p$ becomes negative,
representing a net diversion of male mating effort so complete that it
exceeds the available host female population; in this regime the model
predicts net decline of the host population from competition for male
...
The following result characterizes the dissipative behavior of the RODE system at the population ceiling.
...
To understand how noise affects extinction, we focus on the $p$-subsystem as the driver of dynamics.  When $p$ is small and approaching zero, the logistic factor $L$ approaches $1 - f_\infty - m_\infty > 0$ (where $f_\infty$ and $m_\infty$ are the quasi-steady-state host populations), and the mating term $m$ is approximately constant at its quasi-steady value.  In this regime, the $p$-equation is well approximated by geometric Brownian motion:
\begin{equation}
  dp = (\beta_{\text{eff}} - \delta)p\,dt + \sigma_p p\,dW,
\end{equation}
where $\beta_{\text{eff}} = \gamma\beta L_0 m_0$ is an effective birth rate parameter that depends on the discrimination level and the host quasi-steady state.
...
The It\^{o} Lyapunov exponent measures the linearized growth rate of $\ln p$ near the extinction boundary.  It is a local property of the $p$-subsystem, evaluated under the approximation that the host populations are at quasi-steady state.  The exponent $\lambda_{\text{It\^{o}}} = \beta_{\text{eff}} - \delta - \tfrac{1}{2}\sigma_p^2$ determines whether the parasite population grows or decays exponentially in the neighborhood of $p = 0$.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccl}
    ...
    Initial host females & $\tilde{f}_0$ & 100 & --- \\
    Initial males & $\tilde{m}_0$ & 100 & --- \\
    Initial parasite females & $\tilde{p}_0$ & 10 & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
Figure~\ref{fig:constant_gamma} shows the deterministic dynamics under constant $\gamma$ for three cases: $\gamma < \gamma_{eq}$ (parasite extinction), $\gamma = \gamma_{eq}$ (coexistence), and $\gamma > \gamma_{eq}$ (host then parasite extinction).  Figure~\ref{fig:sigmoid_gamma} shows that when $\gamma$ is a sigmoid function of $p$, the speed of discrimination controls the outcome: slow discrimination ($v = 0.02$) leads to extinction, while fast discrimination ($v = 0.20$) leads to coexistence.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{poecilia_sde/figures/fig02_constant_gamma.png}
  \caption{Constant $\gamma$ deterministic dynamics ($a = 0.8$).  (a)~$\gamma = 0.50 < \gamma_{eq}$: parasite extinction.  (b)~$\gamma = 0.74 = \gamma_{eq}$: coexistence.  (c)~$\gamma = 0.90 > \gamma_{eq}$: host then parasite extinction.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{poecilia_sde/figures/fig07_moment_vs_montecarlo.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{poecilia_sde/figures/fig08_stability_boundary.png}
...
\hline
It\^{o} / common          & 0.256 & 0.384 \\
It\^{o} / independent     & 0.256 & 0.384 \\
Stratonovich / common      & 0.256 & 0.384 \\
Stratonovich / independent & 0.256 & 0.384 \\
\hline
